...
DocBot.prototype._onStart = function() {
    var self = this;
    this._loadBotUser();
    self.subscribedChannels = self._getChannelsByUser();
    this._welcomeMessage();
    //this._connectDb();
    //this._firstRunCheck();
    this._getChannelsByUser();
};
 ...
 * @private
 */
DocBot.prototype._getChannelsByUser = function() {
    console.log('getChannels()');
    console.log('Anzahl von Channels: ' + this.channels.length);
    ...
    console.log('Filtered channels so far:');
...
DocBot.prototype._areKeyWordsUsed = function (keywords, message) {
    ...
    if(keywords.length == 1) {
        isUsed = this.checkWordInMessage(keywords, message);
    } else {
